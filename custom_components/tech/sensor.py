...
async def async_setup_entry(
    ...
    """Set up entry."""
    _LOGGER.debug("Setting up sensor entry, module udid: %s", config_entry.data["module"]["udid"])
...
def map_to_temperature_sensors(zones, api, config_entry):
    """
    Maps the zones to temperature sensors using the provided API and config entry.
    ...
    return map(lambda deviceIndex: TechTemperatureSensor(zones[deviceIndex], api, config_entry), devices)
...
def map_to_humidity_sensors(zones, api, config_entry):
    ...
    return map(
        lambda deviceIndex: TechHumiditySensor(zones[deviceIndex], api, config_entry),
        devices,
    )
...
def map_to_tile_sensors(tiles, api, config_entry):
    ...
    devices_objects = map(
        lambda deviceIndex: TechOutsideTempTile(tiles[deviceIndex], api, config_entry),
        devices_outside_temperature,
    )
...
class TechBatterySensor(SensorEntity):
    ...
    async def async_update(self):
        ...
        device = await self._api.get_zone(self._config_entry.data["module"]["udid"], self._id)
        self.update_properties(device)
...
class TechOutsideTempTile(SensorEntity):
    ...
    async def async_update(self):
        ...
        device = await self._api.get_tile(self._config_entry.data["module"]["udid"], self._id)
        self.update_properties(device)
